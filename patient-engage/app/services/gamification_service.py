...
def _calculate_streak(points: PatientPoints, include_today: bool = True) -> int:
    ...
    current = _utc_now().date() if include_today else _utc_now().date() - timedelta(days=1)
    streak = 0
...
async def award_points(patient_id: str, action: str) -> dict[str, Any]:
    ...
    if normalized_action == "checkin":
        if today not in existing_dates:
            earned += 10
            points.points_breakdown.append(PointsBreakdown(action="checkin", points=10, date=_utc_now()))
            ...
            if points.streak_days and points.streak_days % 7 == 0:
                earned += 50
                points.points_breakdown.append(PointsBreakdown(action="streak_7_days_bonus", points=50, date=_utc_now()))
        ...
        points.points_breakdown.append(PointsBreakdown(action=normalized_action, points=action_points, date=_utc_now()))
