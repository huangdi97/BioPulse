...
class RuntimeCoreTools:
    ...
    def _accumulate_cost(self, ai_resp: dict, step: int) -> None:
        ...
        for target, source in (("prompt_tokens", "prompt_tokens"), ("completion_tokens", "completion_tokens"), ("total_tokens", "total_tokens")):
            self._core._cost_tracker[target] += usage.get(source, 0)
        cost = ai_resp.get("cost") or {
            "model_tier": ai_resp.get("model_tier", "cloud_normal"),
            "input_tokens": usage.get("prompt_tokens", 0),
            "output_tokens": usage.get("completion_tokens", 0),
        }
        self._core._cost_governor.record_step_cost(cost, step)
        summary = self._core.get_cost_usage()
        self._core._cost_tracker["total_cost"] = summary["total_cost"]
        self._core._cost_tracker["step_costs"] = summary["step_costs"]
        db = getattr(self._core, "_agent_db", None)
        if db:
            self._core._cost_governor.record_call(
                agent_name=getattr(self._core, "_trace_id", "unknown"),
