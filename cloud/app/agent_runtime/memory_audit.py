...
class MemoryAuditLogger:
    ...
    def log(self, action: str, agent_key: str, user_id: int, key: str, value: str = "") -> None:
        ...
        if self._db:
            try:
                self._db.execute(
                    "INSERT INTO memory_audit_log (timestamp, action, agent_key, user_id, key, value_preview) "
                    "VALUES (?, ?, ?, ?, ?, ?)",
                    (entry["timestamp"], entry["action"], entry["agent_key"],
                     entry["user_id"], entry["key"], entry["value_preview"]),
                )
                self._db.commit()
