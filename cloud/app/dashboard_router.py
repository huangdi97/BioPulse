...
@router.get("/visit-trends", summary="拜访趋势", description="从visits表按日分组查最近30天拜访趋势，返回拜访日期与次数。")
def visit_trends(
    current_user: dict = Depends(require_scope("visit")),
    service: DashboardService = Depends(),
) -> Any:
...
@router.get("/team-ranks", summary="团队排名", description="从teams表带用户关联查询团队排名，返回团队名称、评分与成员数。")
def team_ranks(
    current_user: dict = Depends(require_scope("visit")),
    service: DashboardService = Depends(),
) -> Any:
...
@router.get("/violations", summary="违规记录", description="从compliance_l2_log表查询违规记录，返回规则ID、名称、严重级别、详情和创建时间。")
def violations(
    current_user: dict = Depends(require_scope("visit")),
    service: DashboardService = Depends(),
) -> Any:
...
@router.get("/research-kpis", summary="科研KPI", description="从research相关表(pi_profiles, products, quotations)查科研KPI，返回KPI名称与数值。")
def research_kpis(
    current_user: dict = Depends(require_scope("visit")),
    service: DashboardService = Depends(),
) -> Any:
...
@router.get("/pi-sources", summary="PI来源统计", description="从pi_profiles表按source字段分组统计PI来源分布。")
def pi_sources(
    current_user: dict = Depends(require_scope("visit")),
    service: DashboardService = Depends(),
) -> Any:
...
@router.get("/product-match-stats", summary="产品匹配统计", description="从product_matches表查匹配统计，返回各分类匹配概览。")
