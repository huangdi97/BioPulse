...
from cloud.shared.columns import (
    TABLE_MDT_SESSIONS_COLS, TABLE_MDT_PARTICIPANTS_COLS,
    TABLE_MDT_OPINIONS_COLS, TABLE_ASYNC_MDT_OPINIONS_COLS,
    TABLE_SOAP_DECISIONS_COLS, TABLE_SOAP_TEMPLATES_COLS,
    TABLE_DECISION_CASES_COLS, TABLE_CAUSAL_ANALYSES_COLS,
    TABLE_CAUSAL_GRAPHS_COLS, TABLE_COUNTERFACTUAL_SCENARIOS_COLS,
    TABLE_CROSS_CASE_INSIGHTS_COLS, TABLE_COLLABORATION_SESSIONS_COLS,
...
class MdtOpinionsRepository(BaseRepository):
    ...
    def build_round_summary(self, session_id: int, round_num: int) -> str:
        ...
        rows = self.db.execute(
            ...
            f"JOIN mdt_participants p ON p.id=o.participant_id "
            f"WHERE o.session_id=? AND o.round_number=?", (session_id, round_num)
        ).fetchall()
...
class SoapDecisionsRepository(BaseRepository):
    ...
    def list_active_filtered(self, status=None, priority=None, tag=None, page=1, page_size=20):
        ...
        return self.paginate(page=page, page_size=page_size, conditions=conditions,
...
class CrossCaseInsightsRepository(BaseRepository):
    ...
    def list_filtered(self, insight_type=None, confidence_min=None, page=1, page_size=20):
        ...
        return self.paginate(page=page, page_size=page_size, conditions=conditions,
                             params=params or None, order_by="confidence DESC")
