...
class VcCredentialsRepository(BaseRepository):
    ...
    def get_by_vc_id(self, vc_id: str):
        row = self.db.execute(
            f"SELECT {', '.join(self.cols)} FROM {self.table_name} WHERE vc_id=?", (vc_id,)
        ).fetchone()
        return dict(row) if row else None
    ...
    def list_filtered(self, issuer_did=None, subject_did=None, credential_type=None, vc_status=None):
        conditions, params = [], []
        if issuer_did:
            conditions.append("issuer_did=?"); params.append(issuer_did)
        if subject_did:
            conditions.append("subject_did=?"); params.append(subject_did)
        if credential_type:
            conditions.append("credential_type=?"); params.append(credential_type)
        if vc_status:
            conditions.append("status=?"); params.append(vc_status)
