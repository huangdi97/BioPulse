...
def seed_soap_decision(conn: sqlite3.Connection) -> None:
    ...
    templates = [
        ("通用SOAP模板", "general",
         _json.dumps({"sections":["Subjective","Objective","Assessment","Plan"],
                      "fields":{"Subjective":"主诉与主观信息","Objective":"客观数据与检查结果",
                                "Assessment":"评估与诊断分析","Plan":"行动计划与策略"}},
                     ensure_ascii=False)),
        ("产品策略SOAP模板", "product_strategy",
         _json.dumps({"sections":["Subjective","Objective","Assessment","Plan"],
                      "fields":{"Subjective":"客户反馈与市场感知","Objective":"销量数据与竞品动态",
                                "Assessment":"产品定位与差异化分析","Plan":"推广策略与资源分配"}},
                     ensure_ascii=False)),
        ("合规评估SOAP模板", "compliance",
         _json.dumps({"sections":["Subjective","Objective","Assessment","Plan"],
                      "fields":{"Subjective":"合规风险感知","Objective":"审计数据与违规记录",
                                "Assessment":"风险等级与影响评估","Plan":"整改方案与培训计划"}},
                     ensure_ascii=False)),
    ]
    conn.executemany(
    ...
    decisions = [
        (1, "竞品A新药上市应对策略决策", "团队反馈客户询问竞品A新药数据频繁，市场部感知到竞争压力增大",
        ...
        (2, "合规推广材料修订决策", "内部抽查发现部分推广材料存在对比性表述，合规团队发出风险预警",
    ...
    opinions = [
        (decision_ids[0], 1, "情报分析师", "根据最新情报，竞品A新药获批后已在5家头部三甲医院完成准入。"
        ...
        (decision_ids[0], 2, "策略规划师", "基于竞品优势在疗效但弱点在适应症范围的判断，建议采取差异化定位策略。"
         "我方应强调'更广泛适应症覆盖+更好安全性记录'作为核心信息，避免与竞品在疗效数据上直接对比。"
         "建议分配60%资源用于学术推广，30%用于数字化触达，10%用于KOL维护。",
         _json.dumps({"资源分配":{"学术推广":0.6,"数字化":0.3,"KOL":0.1}}, ensure_ascii=False),
         "supportive", 0.78,
         _json.dumps(["差异化策略方案.pdf"], ensure_ascii=False)),
        (decision_ids[0], 3, "销售总监", "从落地角度看，差异化策略需要销售团队重新学习话术，需要2-3周培训周期。"
         "同时竞品也在加速推广，时间窗口紧迫。建议简化核心信息到3个关键论点，制作便携式'话术卡片'，"
         "先在一线团队试点2周再全面推广。",
         "", "neutral", 0.65,
         _json.dumps([], ensure_ascii=False)),
        (decision_ids[1], 1, "合规专员", "审计发现的违规问题集中在与竞品相关的陈述中。根源是竞品压力下销售团队在推广中"
         "缺乏足够合规支持材料。建议一方面加强合规培训，更重要的是提供'合规的差异化话术模板'，"
         "让团队在不违规的前提下有效传递产品优势。",
         _json.dumps({"违规分类":{"绝对化用语":3,"疗效对比":2},"根本原因":"竞品压力"}, ensure_ascii=False),
         "supportive", 0.92,
