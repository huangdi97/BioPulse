...
def seed_decision_intel(conn: sqlite3.Connection) -> None:
    ...
    cases = [
        ("竞品A新药上市应对", "对竞品A新药上市后的销售策略效果分析",
         "待评估", 0.3,
         {"product_area": "肿瘤", "competitor": "竞品A", "input_features": {"response_speed": "slow", "data_preparation": "moderate"}},
         ["竞品分析", "策略评估"]),
        ("集采政策应对策略", "集采政策出台后的市场应对策略评估",
         "失败", -0.5,
         {"product_area": "心血管", "policy": "国家集采", "input_features": {"adjustment_speed": "slow", "portfolio_depth": "shallow"}},
         ["政策应对", "集采"]),
        ("学术推广活动效果评估", "针对关键意见领袖精准投放的学术推广效果",
         "成功", 0.8,
         {"product_area": "免疫", "channel": "KOL", "input_features": {"targeting": "precise", "content_quality": "high"}},
         ["学术推广", "KOL"]),
    ]
    ...
    for name, desc, outcome, score, ctx, tags in cases:
        cur = conn.execute(
            ...
            (name, desc, outcome, score,
             _json.dumps(ctx, ensure_ascii=False), _json.dumps(tags, ensure_ascii=False),
             now, now))
        case_id = cur.lastrowid
        drivers = _json.dumps(
            [{"factor": k, "impact": "medium", "direction": "positive" if score > 0 else "negative"}
             for k in ctx.get("input_features", {}).keys()], ensure_ascii=False)
        chain = _json.dumps(
            [{"cause": "策略执行", "effect": outcome, "strength": abs(score)}],
            ensure_ascii=False)
        attr = _json.dumps({k: score / max(1, len(ctx.get("input_features", {})))
                           for k in ctx.get("input_features", {})}, ensure_ascii=False)
        recs = _json.dumps(
            ["继续强化该策略" if score > 0 else "重新评估策略方向"],
            ensure_ascii=False)
        conn.execute(
