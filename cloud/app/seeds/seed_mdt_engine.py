...
def seed_mdt_engine(conn: sqlite3.Connection) -> None:
    ...
    sessions = [
        ("集采政策应对策略辩论", "面对国家集采政策，我们应采取什么样的市场应对策略？",
         "国家医保局持续推进药品集中带量采购，覆盖品种不断扩大。企业面临降价压力和市场份额重新分配。",
         "active", 1, now, now),
        ("新市场准入与产品差异化策略", "新市场准入过程中如何平衡产品差异化和成本控制？",
    ...
    s1_opinions = [
        (session_ids[0], part_ids[0], 1,
         "根据情报分析，集采政策已覆盖65%以上的公立医院市场，降价幅度平均52%。建议加强创新药管线布局，降低对仿制药的依赖。",
         "集采影响范围扩大，创新转型是根本出路", "constructive", 0.85,
         _json.dumps(["集采覆盖率65%","平均降幅52%","创新药转型","仿制药风险"], ensure_ascii=False),
         _json.dumps({"simulated": True}, ensure_ascii=False)),
        (session_ids[0], part_ids[1], 1,
         "策略上建议采取差异化产品定位：一是聚焦未集采的治疗领域开发高价值产品，二是通过学术推广建立品牌护城河，三是考虑探索医联体内部用药目录。",
         "差异化定位三大策略：领域选择、品牌建设、医联体渠道", "positive", 0.90,
         _json.dumps(["差异化定位","学术推广","医联体渠道","品牌护城河"], ensure_ascii=False),
         _json.dumps({"simulated": True}, ensure_ascii=False)),
        (session_ids[0], part_ids[2], 1,
         "从执行层面看，学术推广成本上升约30%，需要评估ROI。建议结合数字化营销工具降低触达成本，同时关注基层市场增量机会。",
         "需要关注执行成本和基层市场机会", "mixed", 0.78,
         _json.dumps(["推广成本上升30%","数字化工具","ROI评估","基层市场"], ensure_ascii=False),
         _json.dumps({"simulated": True}, ensure_ascii=False)),
        (session_ids[0], part_ids[3], 1,
         "复盘历史数据：成功应对集采的企业普遍提前2-3年布局创新产品线，同时在成本控制上做到行业前30%。建议量化各策略的时间成本和预期收益。",
         "历史数据显示提前布局和成本控制是关键成功因素", "analytical", 0.88,
         _json.dumps(["提前2-3年布局","成本控制行业前30%","量化分析","时间成本"], ensure_ascii=False),
         _json.dumps({"simulated": True}, ensure_ascii=False)),
    ]
