...
def seed_s5(conn: sqlite3.Connection) -> None:
    ...
    if count_ts == 0:
        scripts = [
            (
                ...
                _json.dumps(
                    [
                        {"step": 1, "action": "识别竞品弱点"},
                        {"step": 2, "action": "准备差异化证据"},
                        {"step": 3, "action": "制定标准应答"},
                    ],
                    ensure_ascii=False,
                ),
                "mid",
                _json.dumps(["sales_rep", "hcp_outreach"], ensure_ascii=False),
                0.88,
            ),
            (
                "ts:test-002",
                ...
                "mid",
                _json.dumps(["sales_rep", "compliance_checker"], ensure_ascii=False),
                0.92,
            ),
        ]
        for (
    ...
    if count_roi == 0:
        conn.execute(
            ...
            (
                ...
                30000,
                400.0,
                _json.dumps({"method": "simulation"}, ensure_ascii=False),
                now,
            ),
        )
    ...
    new_skills = [
        (
            ...
            _json.dumps(["医学证据生成", "学术回复"], ensure_ascii=False),
            0.85,
            40,
        ),
        (
            ...
            _json.dumps(["医保策略", "招标分析"], ensure_ascii=False),
            0.8,
            40,
        ),
        (
            ...
            _json.dumps(["患者教育", "随访管理"], ensure_ascii=False),
            0.75,
            50,
        ),
    ]
