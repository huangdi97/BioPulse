...
def _call_ai(messages: list[dict], auth_header: str) -> dict:
    ...
    req = urllib.request.Request("http://localhost:8000/ai/chat", data=json.dumps(payload).encode("utf-8"), headers={"Content-Type": "application/json", "Authorization": auth_header}, method="POST")
...
def scan(body: ScanRequest, request: Request, current_user=Depends(require_scope("visit")), db=Depends(get_db)):
    ...
    rules = [{"category": r["category"], "keyword": r["keyword"], "max_value": r["max_value"]} for r in rules_rows]
    result = check_content(body.content, rules)
    violations = result.violations; score = result.score
    ai_analysis = ""
    if body.message_type != "text":
        ...
        messages = [{"role": "system", "content": sys_prompt}, {"role": "user", "content": f"[{body.message_type}]: {body.content}"}]
    ...
    record_id = audit_repo.create({"message_type": body.message_type, "content": body.content, "source_id": body.source_id, "score": score, "risk_level": risk_level, "passed": int(passed), "violations": json.dumps(violations, ensure_ascii=False), "ai_analysis": ai_analysis, "created_by": uid, "created_at": n})
    chain_repo.create({"entity_type": "compliance_audit", "entity_id": str(record_id), "action": "scan", "previous_hash": "", "current_hash": hashlib.sha256(json.dumps({"action": "scan", "record_id": record_id}).encode()).hexdigest(), "payload": json.dumps({"record_id": record_id, "passed": passed, "risk_level": risk_level}, ensure_ascii=False), "source": body.source_id, "created_by": uid, "performed_at": n})
...
def list_records(message_type: Optional[str] = Query(None), risk_level: Optional[str] = Query(None), passed: Optional[int] = Query(None), date_from: Optional[str] = Query(None), date_to: Optional[str] = Query(None), page: int = Query(1, ge=1), page_size: int = Query(20, ge=1, le=100), current_user=Depends(require_scope("visit")), db=Depends(get_db)):
    ...
    if risk_level: conditions.append("risk_level=?"); params.append(risk_level)
    if passed is not None: conditions.append("passed=?"); params.append(passed)
    if date_from: conditions.append("created_at >= ?"); params.append(date_from)
    if date_to: conditions.append("created_at <= ?"); params.append(date_to)
    total, _, rows = repo.paginate(page=page, page_size=page_size, conditions=conditions or None, params=params or None, order_by="created_at DESC")
...
def review_record(record_id: int, body: ReviewRequest, current_user=Depends(require_scope("visit")), db=Depends(get_db)):
    ...
    chain_repo.create({"entity_type": "compliance_audit", "entity_id": str(record_id), "action": "review", "previous_hash": "", "current_hash": hashlib.sha256(json.dumps({"action": "review", "record_id": record_id, "notes": body.review_notes}).encode()).hexdigest(), "payload": json.dumps({"override_passed": body.override_passed, "review_notes": body.review_notes}, ensure_ascii=False), "source": "", "created_by": uid, "performed_at": n})
...
def create_audit_chain(body: AuditChainCreate, current_user=Depends(require_scope("visit")), db=Depends(get_db)):
    ...
    uid = int(current_user["sub"]); n = _now()
    prev_hash = ""
    latest_rows = repo.list_all(conditions=["entity_type=?", "entity_id=?"], params=[body.entity_type, body.entity_id], order_by="performed_at DESC")
    if latest_rows:
    ...
    repo.create({"entity_type": body.entity_type, "entity_id": body.entity_id, "action": body.action, "previous_hash": prev_hash, "current_hash": current_hash, "payload": json.dumps(body.payload, ensure_ascii=False), "metadata": json.dumps(body.metadata, ensure_ascii=False), "source": body.source, "created_by": uid, "performed_at": n})
...
def verify_audit_chain(entity_type: str, entity_id: str, current_user=Depends(require_scope("visit")), db=Depends(get_db)):
    ...
    for i, row in enumerate(rows):
        ...
        if row["previous_hash"] != expected_prev:
            valid = False; broken_at = row["id"]; break
...
def train_correction(record_id: int, request: Request, current_user=Depends(require_scope("visit")), db=Depends(get_db)):
    ...
    messages = [{"role": "system", "content": sys_prompt}, {"role": "user", "content": f"内容类型: {row['message_type']}\n内容: {row['content']}\n违规: {json.dumps(violations, ensure_ascii=False)}\n风险等级: {row['risk_level']}"}]
    auth = request.headers.get("Authorization", "")
    ai_data = _call_ai(messages, auth)
    ai_reply = ai_data.get("reply", "")
    ...
    if isinstance(parsed, dict):
        ...
        title = "培训纠正"; desc = ai_reply[:300]; cat = "general"; sev = "medium"
    uid = int(current_user["sub"]); n = _now()
    corrections_repo.create({"audit_record_id": record_id, "title": title, "description": desc, "category": cat, "severity": sev, "status": "pending", "created_by": uid, "created_at": n})
...
def dashboard(current_user=Depends(require_scope("visit")), db=Depends(get_db)):
    ...
    by_type = db.execute("SELECT message_type, COUNT(*) as cnt, AVG(score) as avg_score FROM compliance_audit_records GROUP BY message_type").fetchall()
    by_risk = db.execute("SELECT risk_level, COUNT(*) as cnt FROM compliance_audit_records GROUP BY risk_level ORDER BY CASE risk_level WHEN 'critical' THEN 0 WHEN 'high' THEN 1 WHEN 'medium' THEN 2 ELSE 3 END").fetchall()
    ...
    trend_7d = db.execute("SELECT DATE(created_at) as d, COUNT(*) as cnt, SUM(passed) as passed_cnt FROM compliance_audit_records WHERE created_at >= ? GROUP BY d ORDER BY d", (week_ago,)).fetchall()
    daily_trend = [{"date": (r["d"] or "")[-5:], "count": (r["cnt"] or 0) - (r["passed_cnt"] or 0)} for r in trend_7d]
    risk_map = {3: "critical", 2: "high", 1: "medium", 0: "low"}
    top_reps = []
    rep_rows = db.execute("SELECT u.username, COUNT(*) as violation_count, MAX(CASE car.risk_level WHEN 'critical' THEN 3 WHEN 'high' THEN 2 WHEN 'medium' THEN 1 ELSE 0 END) as max_risk FROM compliance_audit_records car JOIN users u ON car.created_by = u.id WHERE car.passed = 0 GROUP BY car.created_by ORDER BY violation_count DESC LIMIT 3").fetchall()
    for r in rep_rows:
        top_reps.append({"repName": r["username"], "violationCount": r["violation_count"], "riskLevel": risk_map.get(r["max_risk"], "low")})
    recent_corrections = corrections_repo.list_all(order_by="created_at DESC")[:5]
    return success(data={"todayViolations": today_violations, "weeklyTotal": weekly_total, "processedRate": pass_rate, "highRiskCount": high_risk_count, "dailyTrend": daily_trend, "topCategories": top_categories, "topReps": top_reps, "by_type": [{"message_type": r["message_type"], "count": r["cnt"], "avg_score": round(r["avg_score"] or 0, 2)} for r in by_type], "by_risk": [{"risk_level": r["risk_level"], "count": r["cnt"]} for r in by_risk], "top_violations": [{"violation": v, "count": c} for v, c in top_v], "recent_corrections": recent_corrections})
