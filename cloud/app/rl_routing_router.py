...
@router.post("/route", summary="Route Task")
def route_task(
    body: RouteRequest,
...
@router.post("/pareto-route", summary="Pareto Route")
def pareto_route(
...
@router.post("/log", summary="Log Result")
def log_result(
    body: LogRequest,
    current_user: dict = Depends(require_scope("visit")),
    service: RLRoutingService = Depends(),
...
@router.get("/stats", summary="Get Stats by ID")
def get_stats(
