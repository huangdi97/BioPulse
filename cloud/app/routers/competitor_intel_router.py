...
def create_crawl_job(body: CrawlCreate):
...
def list_crawl_jobs():
    return {"code": 0, "message": "success", "data": list(CRAWL_JOBS.values())}
...
def list_products(status_filter: Optional[str] = Query(None, alias="status")):
...
def update_product(id: str, body: ProductUpdate):
    product = PRODUCTS.get(id)
    if not product:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="product not found")
...
def delete_product(id: str):
    ...
    if not product:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="product not found")
    return {"code": 0, "message": "success", "data": {"deleted": id}}


@router.get("/price/trend", tags=["竞品情报"])
def price_trend(product_id: str = Query(...), days: int = Query(90, ge=1, le=365)):
    return {"code": 0, "message": "success", "data": _price_trend(product_id, days)}


@router.get("/price/anomaly", tags=["竞品情报"])
def price_anomaly(product_id: str = Query(...)):
    trend = _price_trend(product_id, 30)["trend"]
    ...
    return {
        "code": 0,
        "message": "success",
        "data": {"product_id": product_id, "baseline_price": round(average, 2), "anomalies": anomalies},
    }


@router.get("/sentiment", tags=["竞品情报"])
def sentiment(keyword: str = Query(...), days: int = Query(30, ge=1, le=365)):
    return {"code": 0, "message": "success", "data": sentiment_analysis(keyword, days)}


@router.get("/volume", tags=["竞品情报"])
def volume(platform: str = Query("weibo")):
    return {
        "code": 0,
        "message": "success",
        "data": {
...
def compare(product_ids: str = Query(..., description="逗号分隔产品ID，如 prod-001,prod-002")):
    return {"code": 0, "message": "success", "data": competitive_comparison(_split_product_ids(product_ids))}


@router.get("/brief/latest", tags=["竞品情报"])
def latest_brief():
    return {"code": 0, "message": "success", "data": BRIEFS[-1]}
...
def get_brief(brief_id: str):
    ...
    return {"code": 0, "message": "success", "data": brief}


@router.post("/brief/generate", status_code=status.HTTP_201_CREATED, tags=["竞品情报"])
def generate_brief(body: BriefGenerate):
