...
@router.post("/sources", status_code=status.HTTP_201_CREATED, tags=["Market Intel"])
def create_source(
    body: SourceCreate,
...
@router.get("/sources", tags=["Market Intel"])
def list_sources(
...
@router.patch("/sources/{source_id}", tags=["Market Intel"])
def update_source(
    source_id: int,
    body: SourceUpdate,
    current_user=Depends(require_scope("visit")),
...
@router.delete("/sources/{source_id}", tags=["Market Intel"])
def delete_source(
    source_id: int,
    current_user=Depends(require_scope("visit")),
    service: MarketIntelService = Depends(),
...
@router.post("/sources/{source_id}/collect", tags=["Market Intel"])
def collect_source(
    source_id: int,
    current_user=Depends(require_scope("visit")),
    service: MarketIntelService = Depends(),
...
@router.get("/items", tags=["Market Intel"])
def list_items(
...
@router.get("/items/{item_id}", tags=["Market Intel"])
def get_item(
    item_id: int,
    current_user=Depends(require_scope("visit")),
    service: MarketIntelService = Depends(),
...
@router.patch("/items/{item_id}/status", tags=["Market Intel"])
...
@router.post("/items/{item_id}/analyze", tags=["Market Intel"])
def analyze_item(
    item_id: int,
    request: Request,
    current_user=Depends(require_scope("visit")),
...
@router.post("/collect-all", tags=["Market Intel"])
def collect_all(
    current_user=Depends(require_scope("visit")),
    service: MarketIntelService = Depends(),
):
...
@router.get("/dashboard", tags=["Market Intel"])
def dashboard(
    current_user=Depends(require_scope("visit")),
    service: MarketIntelService = Depends(),
):
