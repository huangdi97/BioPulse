...
@router.get("/score", summary="评分查询", description="查询所有记忆的评分")
def get_all_scores(service: SageEngineService = Depends()):
    return success(data=service.score_all_memories())


@router.get("/score/{memory_type}/{memory_id}", summary="评分详情", description="查询指定记忆类型的评分详情")
