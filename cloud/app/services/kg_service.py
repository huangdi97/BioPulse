...
def _bfs_expand(entities_repo: KgEntitiesRepository, relations_repo: KgRelationsRepository,
                seed_eids, max_depth):
...
class KgService(BaseService):
    def create_entity(self, data, user: dict) -> dict:
        ...
        entities_repo.create({
            "entity_id": entity_id, "entity_type": data.entity_type, "name": data.name,
            "aliases": json.dumps(data.aliases, ensure_ascii=False),
            "properties": json.dumps(data.properties, ensure_ascii=False),
            "source_table": data.source_table, "source_row_id": data.source_row_id or 0,
            "confidence": data.confidence, "created_by": int(user["sub"]),
        })
    ...
    def create_relation(self, data) -> dict:
        entities_repo = KgEntitiesRepository(self.db)
        relations_repo = KgRelationsRepository(self.db)
        for label, eid in [("Source", data.source_entity_id), ("Target", data.target_entity_id)]:
            entity = entities_repo.get_by_entity_id(eid)
            if not entity or entity.get("status") != "active":
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"{label} entity not found")
        row_id = relations_repo.create({
            "source_entity_id": data.source_entity_id, "target_entity_id": data.target_entity_id,
            "relation_type": data.relation_type, "weight": data.weight,
            "properties": json.dumps(data.properties, ensure_ascii=False),
    ...
    def search_kg(self, data) -> dict:
        ...
        seeds = [r["entity_id"] for r in entities_repo.search_by_name_and_types(
        ...
        cache_repo.create({
            "query_hash": qhash, "query_text": data.query,
            "result_summary": json.dumps({"entities": len(ent_dict), "relations": len(rel_dict)}, ensure_ascii=False),
            "result_count": len(ent_dict) + len(rel_dict),
        })
        return {"entities": list(ent_dict.values()), "relations": list(rel_dict.values())}
