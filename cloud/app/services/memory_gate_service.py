...
class MemoryGateService(BaseService):
    ...
    def create_gate(self, name: str, source_type: str,
                    importance_threshold: float = 0.5,
                    ttl_days: int = 90,
                    retention_policy: str = "normal") -> dict:
        repo = MemoryGatesRepository(self.db)
        gate_id = repo.create({
            "name": name, "source_type": source_type,
            "importance_threshold": importance_threshold,
            "ttl_days": ttl_days, "retention_policy": retention_policy,
            "created_at": _now(),
        })
        return {"id": gate_id, "name": name, "source_type": source_type,
                "importance_threshold": importance_threshold,
                "ttl_days": ttl_days, "retention_policy": retention_policy}
    ...
    def create_entry(self, title: str, content: str, memory_type: str,
                     source_type: str, source_id: Optional[str],
                     importance: float, context_tags: list[str], uid: int) -> dict:
        ...
        entry_id = repo.create({
            ...
            "source_id": source_id or "", "importance": importance,
            "context_tags": tags, "access_count": 0, "created_by": uid,
    ...
    def list_entries(self, memory_type: Optional[str] = None,
                     importance_min: Optional[float] = None,
                     keyword: Optional[str] = None,
                     page: int = 1, page_size: int = 20) -> tuple[int, int, list[dict]]:
        repo = MemoryEntriesRepository(self.db)
    ...
    def recall(self, query: str, memory_types: list[str],
               min_importance: float, max_results: int) -> dict:
        ...
        now = _now()
        recall_repo.create({
            "query_text": query, "memory_ids": json.dumps(ids),
    ...
    def auto_store(self, source_type: str, source_id: str, uid: int,
        ...
        if stored:
            ...
            entry_repo.create({
                "title": source_title, "content": source_content,
                "memory_type": source_type, "source_type": source_type,
                "source_id": source_id, "importance": importance,
                "context_tags": tags, "access_count": 0,
    ...
    def dashboard(self) -> dict:
        ...
        by_type = [{"memory_type": r["memory_type"], "count": r["cnt"],
                     "avg_importance": round(r["avg_imp"], 4)} for r in type_rows]
