...
def _call_deepseek(messages: list, temperature: float, max_tokens: int) -> dict:
    ...
    req = urllib.request.Request(DEEPSEEK_URL, data=req_data,
...
class RouteService(BaseService):
    def create_rule(self, name: str, priority: int, condition_field: str,
                    condition_operator: str, condition_value: str,
                    target_role_id: int, fallback_role_id: Optional[int],
                    max_tokens: int, temperature: float, created_by: int) -> dict:
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        rules_repo = RouteRulesRepository(self.db)
        rule_id = rules_repo.create({
            "name": name, "priority": priority,
            "condition_field": condition_field, "condition_operator": condition_operator,
            "condition_value": condition_value, "target_role_id": target_role_id,
            "fallback_role_id": fallback_role_id, "max_tokens": max_tokens,
            "temperature": temperature, "created_by": created_by,
    ...
    def update_rule(self, rule_id: int, name: Optional[str] = None,
    ...
    def execute_route(self, input_text: str, uid: int, source: str) -> dict:
        ...
        if matched is None:
            ...
            return {
                "role_name": "", "response": "No matching rule found.",
                "confidence": 0.0, "latency_ms": latency_ms, "matched_rule": None,
            }
        ...
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        logs_repo.create({
            "input_text": input_text, "matched_rule_id": matched["id"],
            "matched_rule_name": matched["name"], "assigned_role_id": role_id,
            "assigned_role_name": role_name, "confidence": confidence,
            "response_summary": reply[:500], "tokens_used": tokens_used,
    ...
    def list_logs(self, role_id: Optional[int] = None,
    ...
    def get_dashboard(self) -> dict:
        ...
        if total == 0:
            return {
                "total_executions": 0, "role_distribution": [],
                "avg_latency_ms": 0, "recent_logs": [],
            }
        rows = logs_repo.role_distribution()
        role_dist = [{"role_id": r["assigned_role_id"], "role_name": r["assigned_role_name"],
                      "count": r["cnt"], "percentage": round(r["cnt"] / total * 100, 2)} for r in rows]
