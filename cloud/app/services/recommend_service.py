...
class RecommendService(BaseService):
    def create_profile(
        ...
        row = profiles_repo.upsert_profile(
            user_id, persona_type, specialization, experience_level, pct, tags
        )
        return row
    ...
    def update_profile(
        ...
        if preferred_content_types is not None:
            updates["preferred_content_types"] = json.dumps(
                preferred_content_types, ensure_ascii=False
            )
    ...
    def generate_recommendations(
        self, user_id: int, rec_types: list, limit: int
    ) -> list:
        behaviors_repo = UserBehaviorsRepository(self.db)
        ...
        if behavior_count >= 3:
            ...
            if top:
                hot = behaviors_repo.top_targets_by_user_action(
                    user_id, top["action_type"], limit
                )
