...
def _call_ai(system_prompt: str, user_prompt: str) -> str:
    ...
    req = urllib.request.Request(
        DEEPSEEK_URL, data=json.dumps(req_body).encode("utf-8"),
...
class HcpSandboxService(BaseService):
    def create_profile(self, name: str, title: str, hospital: str,
                       department: str, specialty: str, city: str,
                       tier: str, traits: dict, prescription_volume: float,
                       influence_score: float, digital_engagement: float,
                       user_id: int) -> dict:
        repo = HcpProfilesRepository(self.db)
        data = {
            "name": name, "title": title, "hospital": hospital,
            "department": department, "specialty": specialty, "city": city,
            "tier": tier, "traits": json.dumps(traits, ensure_ascii=False),
            "prescription_volume": prescription_volume, "influence_score": influence_score,
            "digital_engagement": digital_engagement, "created_by": user_id,
        }
        row_id = repo.create(data)
        return repo.get_by_id(row_id)

    def list_profiles(self, tier: Optional[str] = None,
                      specialty: Optional[str] = None,
                      city: Optional[str] = None,
                      page: int = 1, page_size: int = 20) -> PaginatedResponse:
        repo = HcpProfilesRepository(self.db)
    ...
    def update_profile(self, hcp_id: int, name: Optional[str] = None,
                       title: Optional[str] = None, hospital: Optional[str] = None,
                       department: Optional[str] = None, specialty: Optional[str] = None,
                       city: Optional[str] = None, tier: Optional[str] = None,
        ...
        field_map = {
            "name": name, "title": title, "hospital": hospital,
            "department": department, "specialty": specialty, "city": city,
            "tier": tier, "prescription_volume": prescription_volume,
            "influence_score": influence_score, "digital_engagement": digital_engagement,
            "is_active": is_active,
        }
    ...
    def create_interaction(self, hcp_id: int, interaction_type: str,
                           content: str, response: str, outcome: str,
                           strategy_used: str, conducted_at: Optional[str],
                           user_id: int) -> dict:
        profiles_repo = HcpProfilesRepository(self.db)
        interactions_repo = HcpInteractionsRepository(self.db)
        if not profiles_repo.get_by_id(hcp_id):
            raise HTTPException(status.HTTP_404_NOT_FOUND, detail="HCP profile not found")
        data = {
            "hcp_id": hcp_id, "interaction_type": interaction_type,
            "content": content, "response": response, "outcome": outcome,
            "strategy_used": strategy_used, "conducted_by": user_id,
            "conducted_at": conducted_at,
        }
        row_id = interactions_repo.create(data)
        return interactions_repo.get_by_id(row_id)

    def list_interactions(self, hcp_id: int, page: int = 1,
    ...
    def simulate(self, hcp_id: int, scenario: str, strategy: str,
                 user_id: int) -> dict:
        profiles_repo = HcpProfilesRepository(self.db)
        interactions_repo = HcpInteractionsRepository(self.db)
        simulations_repo = HcpSimulationsRepository(self.db)
        ...
        try:
            ...
            result = {"expected_outcome": "", "confidence": 0.5, "suggested_approach": "",
                      "key_concerns": "", "recommended_topics": "", "risk_factors": ""}
        sim_data = json.dumps({"scenario": scenario, "strategy": strategy,
                               "profile_id": hcp_id}, ensure_ascii=False)
        data = {
            "hcp_id": hcp_id, "scenario": scenario, "strategy": strategy,
            ...
            "simulation_data": sim_data, "status": "completed", "created_by": user_id,
        }
        row_id = simulations_repo.create(data)
        return simulations_repo.get_by_id(row_id)

    def list_simulations(self, hcp_id: Optional[int] = None,
    ...
    def dashboard(self) -> dict:
        ...
        return {"total_hcp": total, "tier_distribution": tier_dist,
