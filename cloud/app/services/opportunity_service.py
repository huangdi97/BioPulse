...
class OpportunityService(BaseService):
    def _stage_probability(self, stage: str) -> int:
        mapping = {"lead": 10, "qualify": 30, "proposal": 50, "negotiation": 75, "won": 100, "lost": 0}
        return mapping.get(stage, 0)

    def _validate_stage_transition(self, current: str, target: str) -> None:
        if current in TERMINAL_STAGES:
            raise HTTPException(status.HTTP_400_BAD_REQUEST, detail="Cannot transition from a terminal stage")
    ...
    def create_opportunity(self, customer_id: int, name: str, description: str,
                           stage: str, estimated_value: float, actual_value: float,
                           assigned_to: Optional[int], close_date: Optional[str],
                           notes: str, user_id: int) -> dict:
        cust_repo = CustomersRepository(self.db)
        ...
        if stage not in VALID_STAGES:
            raise HTTPException(status.HTTP_400_BAD_REQUEST, detail=f"Invalid stage. Must be one of: {VALID_STAGES}")
    ...
    def list_opportunities(self, stage: Optional[str] = None,
    ...
    def update_opportunity(self, opp_id: int, name: Optional[str] = None,
        ...
        if stage is not None:
            ...
            current_row = self.db.execute(
                f"SELECT {placeholders} FROM {opp_repo.table_name} WHERE id=?", (opp_id,)
            ).fetchone()
            if current_row:
    ...
    def transition_stage(self, opp_id: int, stage: str,
