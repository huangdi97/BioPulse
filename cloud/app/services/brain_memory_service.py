...
class BrainMemoryService(BaseService):
    def working_set(self, session_id: str, slot_key: str,
                    slot_value: str, slot_type: str, ttl_seconds: int) -> dict:
        n = _now()
        expires_at = (datetime.now() + timedelta(seconds=ttl_seconds)).strftime("%Y-%m-%d %H:%M:%S")
        wm = WorkingMemoryRepository(self.db)
        wm.db.execute("INSERT OR REPLACE INTO working_memory "
        ...
        wm.db.commit()
        return {"session_id": session_id, "slot_key": slot_key,
                "slot_value": slot_value, "expires_at": expires_at}

    def working_get(self, session_id: str, slot_key: Optional[str] = None) -> dict:
        wm = WorkingMemoryRepository(self.db)
        if slot_key:
            row = wm.db.execute("SELECT * FROM working_memory WHERE session_id=? AND slot_key=? AND expires_at>?",
                                 (session_id, slot_key, _now())).fetchone()
            if not row:
                raise _n404("Slot")
            return {"data": dict(row)}
        rows = wm.db.execute("SELECT * FROM working_memory WHERE session_id=? AND expires_at>?",
                              (session_id, _now())).fetchall()
    ...
    def working_refresh(self, session_id: str) -> dict:
        wm = WorkingMemoryRepository(self.db)
        rows = wm.db.execute("SELECT id, ttl_seconds FROM working_memory WHERE session_id=?",
                              (session_id,)).fetchall()
        for r in rows:
    ...
    def episodic_store(self, event_type: str, title: str,
                       description: str, context: dict,
                       outcome: str, valence: float,
                       intensity: float, involved_agents: list[str],
                       related_entity_type: str,
                       related_entity_id: Optional[int],
                       uid: str) -> dict:
        n = _now()
        em = EpisodicMemoryRepository(self.db)
        new_id = em.create({"event_type": event_type, "title": title,
            "description": description, "context": json.dumps(context, ensure_ascii=False),
            "outcome": outcome, "valence": valence, "intensity": intensity,
            ...
            "created_by": uid, "created_at": n})
        interference = 0
        if related_entity_id or event_type:
            conds, p = [], []
            if related_entity_id and related_entity_type:
                conds.append("related_entity_id=? AND related_entity_type=?"); p += [related_entity_id, related_entity_type]
            if event_type:
                conds.append("event_type=?"); p.append(event_type)
            for old in em.db.execute(f"SELECT valence FROM episodic_memory WHERE id!=? AND ({' OR '.join(conds)})", [new_id] + p):
                if (old["valence"] or 0.0) * (valence or 0.0) < 0:
                    interference += 1
        if interference:
            MemoryConsolidationLogRepository(self.db).create({
                "consolidation_type": "interference_forgetting", "source_table": "episodic_memory",
                "item_count": interference, "items_superseded": interference, "status": "completed",
                "details": json.dumps({"new_id": new_id, "triggered_by": uid}, ensure_ascii=False),
                "created_at": n})
        return {"id": new_id, "event_type": event_type, "title": title,
                "description": description, "outcome": outcome, "valence": valence,
                "intensity": intensity, "involved_agents": involved_agents,
                "related_entity_type": related_entity_type, "related_entity_id": related_entity_id}

    def episodic_list(self, event_type: Optional[str] = None,
                      outcome: Optional[str] = None,
                      date_from: Optional[str] = None,
                      date_to: Optional[str] = None, page: int = 1, page_size: int = 20) -> dict:
        c, p = ["1=1"], []
        if event_type:
            c.append("event_type=?"); p.append(event_type)
        if outcome:
            c.append("outcome=?"); p.append(outcome)
        if date_from:
            c.append("created_at>=?"); p.append(date_from)
        if date_to:
            c.append("created_at<=?"); p.append(date_to)
        total, pages, items = EpisodicMemoryRepository(self.db).paginate(
            page=page, page_size=page_size, conditions=c, params=p, order_by="created_at DESC")
    ...
    def episodic_detail(self, memory_id: int) -> dict:
        ...
        MemoryConsolidationLogRepository(self.db).create({
            ...
            "source_table": "episodic_memory", "item_count": 1, "status": "completed",
            "details": json.dumps({"memory_id": memory_id}, ensure_ascii=False),
            "created_at": _now()})
        return row

    def episodic_consolidate(self, memory_id: int,
        ...
        messages = [
            {"role": "system", "content": "你是一名记忆巩固专家。请根据输入的情景记忆内容，提炼核心洞察，生成结构化的记忆条目。"
             "输出纯JSON（不要markdown代码块）：{\"title\":\"洞察标题\",\"content\":\"详细内容\",\"memory_type\":\"insight\","
             "\"importance\":0.8,\"context_tags\":[\"标签1\",\"标签2\"]}"},
            {"role": "user", "content": f"事件类型: {row['event_type']}\n标题: {row['title']}\n"
        ...
        try:
            ...
            insight = {"title": row["title"], "content": reply or row["description"],
                       "memory_type": "insight", "importance": 0.5, "context_tags": []}
        ...
        return {
            "memory_id": memory_id, "entry_id": entry_id,
            "title": title, "memory_type": memory_type, "importance": importance,
        }
    ...
    def dashboard(self) -> dict:
        ...
        active_sessions = wm_repo.db.execute(
            "SELECT COUNT(DISTINCT session_id) FROM working_memory WHERE expires_at > ?", (now_val,)
        ).fetchone()[0]
    ...
    def semantic_abstract(self, source_type: str, source_id: str,
                          abstraction_level: str, auth_header: str = "") -> dict:
        rows = EpisodicMemoryRepository(self.db).db.execute(
            "SELECT * FROM episodic_memory WHERE related_entity_type=? AND related_entity_id=? ORDER BY created_at DESC",
            (source_type, int(source_id))).fetchall()
        if not rows:
        ...
        try:
            ...
            d = {"title": f"Semantic-{source_type}", "content": reply, "context_tags": []}
        n = _now()
        eid = MemoryEntriesRepository(self.db).create({
            "title": d.get("title", ""), "content": d.get("content", txt[:200]),
            "memory_type": "semantic", "source_type": source_type,
            "source_id": str(source_id), "importance": 0.7,
            "context_tags": json.dumps(d.get("context_tags", []), ensure_ascii=False),
            "access_count": 0, "created_by": 0, "created_at": n, "updated_at": n})
        return {"id": eid, "title": d.get("title", ""), "content": d.get("content", "")}

    def semantic_search(self, query: str, limit: int = 10) -> dict:
        kw = f"%{query}%"
        rows = MemoryEntriesRepository(self.db).db.execute(
    ...
    def procedural_learn(self, pattern_name: str, trigger_conditions: str,
                         action_sequence: str, success_rate: float) -> dict:
        n = _now()
        content = json.dumps({"trigger_conditions": trigger_conditions,
                              "action_sequence": action_sequence,
                              "success_rate": success_rate}, ensure_ascii=False)
        eid = MemoryEntriesRepository(self.db).create({
            "title": pattern_name, "content": content,
            "memory_type": "procedural", "source_type": "procedural_learn",
            "source_id": pattern_name, "importance": min(max(float(success_rate), 0.0), 1.0),
            "context_tags": "[]", "access_count": 0, "created_by": 0,
    ...
    def procedural_recall(self, trigger_context: str) -> dict:
        rows = MemoryEntriesRepository(self.db).db.execute(
            ...
            "ORDER BY importance DESC LIMIT 3").fetchall()
        items = []
        for r in rows:
            try:
            ...
            items.append({"id": r["id"], "title": r["title"],
                          "pattern_data": c, "success_rate": r["importance"]})
