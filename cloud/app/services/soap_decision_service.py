...
from cloud.app.repositories import (
    SoapTemplatesRepository, SoapDecisionsRepository, AsyncMdtOpinionsRepository,
)
...
class SoapDecisionService(BaseService):
    def create_template(self, name: str, category: str, description: str,
                        structure: dict, created_by) -> dict:
    ...
    def create_decision(self, title: str, template_id: Optional[int],
                        subjective: str, objective: str, assessment: str,
                        plan: str, priority: str, tags: list,
                        created_by) -> dict:
        templates_repo = SoapTemplatesRepository(self.db)
    ...
    def list_decisions(self, status: Optional[str] = None,
        ...
        return PaginatedResponse(
            ...
            total=total, page=page, page_size=page_size, total_pages=tp)
    ...
    def update_decision(self, decision_id: int, subjective: Optional[str] = None,
    ...
    def add_opinion(self, decision_id: int, opinion: str, stance: str,
                    supporting_data: str, confidence: float,
                    attachments: list, user_id, role: str) -> dict:
        decisions_repo = SoapDecisionsRepository(self.db)
        ...
        opinion_id = opinions_repo.create({
            "decision_id": decision_id, "contributor_id": user_id,
            "contributor_role": role,
            "opinion": opinion, "supporting_data": supporting_data,
    ...
    def finalize_decision(self, decision_id: int, decision_summary: str,
                          user_id) -> dict:
        decisions_repo = SoapDecisionsRepository(self.db)
        row = decisions_repo.get_by_id(decision_id)
        if not row or not row.get("is_active"):
        ...
        decisions_repo.update(decision_id, {"decision_summary": decision_summary, "status": "decided",
                                             "decided_by": user_id})
        self.db.execute(
            "UPDATE soap_decisions SET decided_at=NOW(), "
            "updated_at=NOW() WHERE id=?", (decision_id,)
        )
        self.db.commit()
        row = decisions_repo.get_by_id(decision_id)
        return _row(row, ["tags"])
