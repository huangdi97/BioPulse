...
from cloud.app.repositories import MarketIntelItemsRepository, MarketIntelSourcesRepository
...
def itd(row, parse_ai=False) -> dict:
    d = {"id": row["id"], "source_id": row["source_id"], "title": row["title"],
         "summary": row["summary"], "content": row["content"], "url": row["url"],
         "item_type": row["item_type"], "relevance_tags": json.loads(row["relevance_tags"]),
         "impact_level": row["impact_level"], "status": row["status"],
         "ai_analysis": row["ai_analysis"], "collected_at": row["collected_at"],
...
def _sim(src_name, kw, idx):
    return (f"【{src_name}】关键词「{kw}」相关情报第{idx}条",
...
def _do_collect(repo, src, user_id):
    ...
    for i in range(3):
        ...
        repo.create_raw({
            ...
            "content": content, "item_type": src["source_type"],
            "impact_level": ["low", "medium", "high"][i % 3], "status": "unread",
            "collected_at": now, "created_by": user_id,
            "created_at": now, "updated_at": now,
        })


class MarketIntelService(BaseService):
    def create_source(self, name: str, source_type: str, target_keywords: list,
                      user_id: int) -> dict:
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    def list_sources(self, source_type: Optional[str] = None,
        ...
        if is_active is not None:
            conditions.append("is_active=?"); params.append(is_active)
        rows = sources_repo.list_all(conditions=conditions or None, params=params or None,
                                     order_by="created_at DESC")
        return [sd(r) for r in rows]

    def update_source(self, source_id: int, name: Optional[str] = None,
    ...
    def list_items(self, item_type: Optional[str] = None,
        ...
        items_repo = MarketIntelItemsRepository(self.db)
        conds, pars = [], []
        if item_type: conds.append("mi.item_type=?"); pars.append(item_type)
        if status_filter: conds.append("mi.status=?"); pars.append(status_filter)
        if impact_level: conds.append("mi.impact_level=?"); pars.append(impact_level)
        if keyword:
            conds.append("(mi.title LIKE ? OR mi.summary LIKE ? OR mi.content LIKE ?)")
            lk = f"%{keyword}%"; pars.extend([lk, lk, lk])
        if date_from: conds.append("mi.collected_at>=?"); pars.append(date_from)
        if date_to: conds.append("mi.collected_at<=?"); pars.append(date_to)
        total, rows = items_repo.list_with_source(
            conditions=conds or None, params=pars or None,
            page=page, page_size=page_size)
        items = []
        for r in rows:
            d = itd(r); d["source_name"] = r.get("source_name"); items.append(d)
    ...
    def analyze_item(self, item_id: int, request: Request) -> dict:
        ...
        payload = {
            "messages": [
                {"role": "system", "content": "You are a market intelligence analyst. Output analysis in JSON format."},
        ...
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        items_repo.update_fields(item_id, {
    ...
    def dashboard(self) -> dict:
        ...
        return {"total_items": t, "unread_count": u, "by_type": bt, "by_impact": bi,
                "recent_critical": rc, "trend_7d": td}
