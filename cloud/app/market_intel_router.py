...
def create_source(body: SourceCreate, current_user=Depends(require_scope("visit")),
...
def list_sources(
    ...
    current_user=Depends(require_scope("visit")), service: MarketIntelService = Depends(),
):
...
def delete_source(source_id: int, current_user=Depends(require_scope("visit")),
                  service: MarketIntelService = Depends()):
...
def collect_source(source_id: int, current_user=Depends(require_scope("visit")),
                   service: MarketIntelService = Depends()):
...
def get_item(item_id: int, current_user=Depends(require_scope("visit")),
             service: MarketIntelService = Depends()):
...
def collect_all(current_user=Depends(require_scope("visit")), service: MarketIntelService = Depends()):
    return success(data=service.collect_all(int(current_user["sub"])))


@router.get("/dashboard")
def dashboard(current_user=Depends(require_scope("visit")), service: MarketIntelService = Depends()):
