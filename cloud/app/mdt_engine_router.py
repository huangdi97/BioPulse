...
def _parse_json(raw: str, default=None):
    ...
    except (json.JSONDecodeError, TypeError): return default if default is not None else raw
...
def create_session(body: SessionCreate, request: Request,
    ...
    sid = sessions_repo.create({
        "title": body.title, "question": body.question, "context": body.context,
        "status": "active", "created_by": uid, "created_at": n, "updated_at": n,
    })
    parts = body.participants
    if not parts:
        roles = roles_repo.list_active(limit=5)
        parts = [ParticipantDef(agent_role_id=r["id"], role_name=r["name"], stance="neutral", vote_weight=1.0)
                 for r in roles]
    for p in parts:
