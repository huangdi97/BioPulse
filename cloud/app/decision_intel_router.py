...
def create_case(body: CaseCreate, request: Request,
    ...
    row = service.create_case(
        name=body.name, pipeline_run_id=body.pipeline_run_id,
        description=body.description, outcome=body.outcome,
        outcome_score=body.outcome_score, context=body.context,
        tags=body.tags, uid=uid,
    )
    return success(data=row)


@router.get("/cases")
def list_cases(outcome_score_min: Optional[float] = Query(None),
...
def update_case(case_id: int, body: CaseUpdate,
    ...
    row = service.update_case(
        ...
        outcome=body.outcome, outcome_score=body.outcome_score,
        context=body.context, tags=body.tags,
    )
    return success(data=row)
...
def analyze_case(case_id: int, body: AnalyzeRequest,
    ...
    result = service.analyze_case(
        case_id=case_id, custom_question=body.custom_question,
...
def reflect(body: ReflectRequest, request: Request,
    ...
    result = service.reflect(
        filter_tags=body.filter_tags, max_cases=body.max_cases,
        auth_header=auth_header,
    )
    return success(data=result)
...
@router.get("/insights")
def list_insights(insight_type: Optional[str] = Query(None),
...
def update_insight(insight_id: int, body: InsightUpdate,
    ...
    row = service.update_insight(
        ...
        summary=body.summary, confidence=body.confidence,
