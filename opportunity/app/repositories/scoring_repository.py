...
class StatsRepository:
    ...
    def _build_where(self, conditions: list, params: list, start_date: str | None, end_date: str | None) -> None:
        if start_date:
    ...
    def get_totals(self, start_date: str | None = None, end_date: str | None = None):
        ...
        row = self.db.execute(
            f"SELECT COUNT(*), COALESCE(SUM(estimated_value), 0) FROM {self.table} {where}", params
        ).fetchone()
        return row[0], row[1]
