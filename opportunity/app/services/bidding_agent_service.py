...
class BiddingAgentService(BaseService):
    def _call_llm(self, auth_header: str, prompt: str) -> str:
        req_body = {"messages": [{"role": "user", "content": prompt}], "temperature": 0.3, "max_tokens": 2048}
    ...
    def _run_scan_for_config(self, auth_header: str, config: dict) -> dict:
        ...
        for bid in bids:
            ...
            self.db.execute(
                ...
                (title, bid.get("hospital", "") or bid.get("医院", ""), bid.get("department", "") or bid.get("科室", ""), bid.get("product_category", "") or bid.get("产品类别", ""), bid.get("budget", 0) or bid.get("预算", 0), bid.get("deadline", "") or bid.get("截止日期", ""), 1, now, now),
            )
            parsed += 1
            if config.get("auto_notify") and config.get("notify_to"):
                try:
                    from shared.notification_client import send_notification
                    send_notification(self.db, config["notify_to"], f"新招标: {title}", f"关键词: {keywords}", "bidding_agent", "bidding_info", None, {"config_id": config["id"]})
                except Exception:
                    pass
        return {"items_found": found, "items_parsed": parsed}

    def _log_scan_result(self, config_id: int, status_: str, found: int, parsed: int, err: Optional[str], started: str):
    ...
    def trigger_scan(self, auth_header: str) -> dict:
        ...
        for cfg in configs:
            ...
            try:
                ...
                self._log_scan_result(config["id"], "success", scan_result["items_found"], scan_result["items_parsed"], None, started_at)
            except Exception as e:
                errors.append({"config_id": config["id"], "error": str(e)})
                self._log_scan_result(config["id"], "failed", 0, 0, str(e), started_at)
        return {"total_found": total_found, "total_parsed": total_parsed, "errors": errors}

    def get_agent_status(self) -> dict:
        last = self.db.execute("SELECT started_at FROM bidding_agent_log ORDER BY id DESC LIMIT 1").fetchone()
        stats = self.db.execute("SELECT COUNT(*) as total, SUM(CASE WHEN run_status='success' THEN 1 ELSE 0 END) as success FROM bidding_agent_log").fetchone()
    ...
    def list_agent_logs(self, page: int, page_size: int) -> tuple:
        ...
        rows = self.db.execute("SELECT * FROM bidding_agent_log ORDER BY id DESC LIMIT ? OFFSET ?", [page_size, offset]).fetchall()
...
请返回JSON格式: {{"opportunity_assessment": "...", "product_suggestions": ["..."], "risk_factors": ["..."], "next_steps": ["..."]}}"""
        ...
        try:
            ...
            analysis = {"opportunity_assessment": reply[:200], "product_suggestions": [], "risk_factors": [], "next_steps": []}
        now = datetime.now(timezone.utc).isoformat()
        self.db.execute("UPDATE bidding_info SET analysis = ?, updated_at = ? WHERE id = ?", (json.dumps(analysis, ensure_ascii=False), now, bidding_id))
        self.db.commit()
        return analysis
    ...
    def run_scheduled_scan(self) -> None:
        ...
        for cfg in configs:
            ...
            try:
                ...
                self._log_scan_result(config["id"], "success", result["items_found"], result["items_parsed"], None, started_at)
            except Exception as e:
                self._log_scan_result(config["id"], "failed", 0, 0, str(e), started_at)
            self.db.commit()
