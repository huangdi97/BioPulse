...
router = APIRouter(prefix="/coach/digital-human")
...
@router.post("/sessions", summary="创建会话", description="创建新的数字人会话", tags=["陪练"])
def create_session(
    body: SessionCreate,
...
@router.post("/sessions/{session_id}/message", summary="发送消息", description="向数字人发送消息并获取回复", tags=["陪练"])
def send_message(
    session_id: int,
    body: MessageSend,
    service: DigitalHumanService = Depends(),
...
@router.get("/sessions/{session_id}", summary="会话详情", description="获取数字人会话详情", tags=["陪练"])
def get_session(
    session_id: int,
    service: DigitalHumanService = Depends(),
    current_user: dict = Depends(require_scope("visit")),
...
@router.post("/sessions/{session_id}/adjust-difficulty", summary="调整难度", description="动态调整数字人会话的难度", tags=["陪练"])
...
@router.post("/sessions/{session_id}/end", summary="结束会话", description="结束当前数字人会话", tags=["陪练"])
def end_session(
    session_id: int,
    service: DigitalHumanService = Depends(),
    current_user: dict = Depends(require_scope("visit")),
...
@router.get("/sessions", summary="会话列表", description="获取当前用户的数字人会话列表", tags=["陪练"])
def list_sessions(
...
@router.post("/sessions/{session_id}/save-memory", summary="保存记忆", description="保存数字人训练记忆数据", tags=["陪练"])
...
@router.get("/benchmarks", summary="基准数据", description="获取基准评分数据", tags=["陪练"])
def get_benchmarks(
    service: DigitalHumanMemoryService = Depends(),
    current_user: dict = Depends(require_scope("visit")),
) -> JSONResponse:
...
@router.get("/sessions/{session_id}/benchmark", summary="基准对比", description="将会话数据与基准数据进行对比分析", tags=["陪练"])
...
@router.post("/sessions/{from_id}/transfer-to/{to_id}", summary="迁移技能", description="将会话中学到的技能迁移至目标会话", tags=["陪练"])
def transfer_skills(
...
@router.post("/sessions/{session_id}/voice-input", summary="语音输入", description="多模态语音输入接口（预留功能）", tags=["陪练"])
...
@router.post("/sessions/{session_id}/video-input", summary="视频输入", description="多模态视频输入接口（预留功能）", tags=["陪练"])
