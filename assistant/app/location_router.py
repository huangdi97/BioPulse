...
@router.post("/hcp/{hcp_id}/locations", summary="创建位置", description="为指定HCP创建新的地址位置。", tags=["路径优化"])
def create_location(
    hcp_id: int,
    body: LocationCreate,
...
@router.get("/hcp/{hcp_id}/locations", summary="查询位置列表", description="获取指定HCP的所有地址位置列表。", tags=["路径优化"])
def list_locations(
    hcp_id: int,
    service: LocationService = Depends(),
    current_user: dict = Depends(require_scope("visit")),
...
@router.patch("/locations/{location_id}", summary="更新位置", description="更新指定位置的字段信息。", tags=["路径优化"])
def update_location(
    location_id: int,
    body: LocationUpdate,
    service: LocationService = Depends(),
...
@router.delete("/locations/{location_id}", summary="删除位置", description="删除指定的位置记录。", tags=["路径优化"])
def delete_location(
    location_id: int,
    service: LocationService = Depends(),
    current_user: dict = Depends(require_scope("visit")),
...
@router.post("/routes/optimize", summary="优化路线", description="根据起点和HCP列表优化行程路线。", tags=["路径优化"])
def optimize_route(
    body: RouteRequest,
    service: LocationService = Depends(),
    current_user: dict = Depends(require_scope("visit")),
