...
@pytest.fixture(autouse=True)
def _mock_offline_cloud_check(monkeypatch):
    monkeypatch.setattr(
        "assistant.app.services.offline_service.OfflineService._check_cloud",
        lambda self: False,
    )
    monkeypatch.setattr(
        "assistant.app.services.offline_service.OfflineService.sync_pending",
        lambda self, limit=50: {"synced_count": 0, "failed_count": 0, "errors": []},
    )
    yield
