...
└──────────────────┘     └──────────────────────┘     └──────────────────┘
           ▼                                                       ▼
 ┌──────────────────┐                                 ┌──────────────────┐
 │    Verifier       │                                 │   State Snapshot  │
 │  结果验证 → 通过/否  │                                 │   检查点 + 恢复    │
 └──────────────────┘                                 └──────────────────┘

> **注：** `reflector.py` / `Reflector` 已移除，反思逻辑已整合至 `runtime_core.py` 与 `runtime_helpers.py`。
```
...
| ~~`reflector.py`~~ | ~~`Reflector`~~ | 已移除，反思逻辑已整合至 `runtime_core.py` / `runtime_helpers.py` |
...
| ~~`unified_memory_service.py`~~ | 已移除，跨端记忆聚合已整合至 `memory_consolidation_service.py` / `brain_orchestrator_service.py` |
...
| ADR-0004 | Agent 通信平面抽象 | 抽象 Agent 间通信接口，支持 A2A/MCP 协议切换 |
| ADR-0005 | 项目线严格分离 | 问年与三重对抗项目不混入同一文档/会话 |
| ADR-0006 | Vibecoding 工作流 | Write→TG→Confirm→OpenCode |
| ADR-0007 | 拆分后全量测试 | 大文件拆分后必须跑全量测试验证 |
