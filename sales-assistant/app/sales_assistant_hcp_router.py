...
from sales_assistant.app.hcp_models import (
    HcpCreate,
    HcpOut,
    HcpUpdate,
    ProductCreate,
...
@router.post("/hcp", summary="创建HCP", description="创建HCP信息", tags=["HCP"])
def create_hcp(
    body: HcpCreate,
...
@router.get("/hcp", summary="HCP列表", description="获取HCP列表", tags=["HCP"])
def list_hcp(
...
@router.get("/hcp/graph", summary="关系图谱", description="获取HCP关系图谱", tags=["HCP"])
def get_graph(
...
@router.get("/hcp/{hcp_id}", summary="HCP详情", description="获取指定HCP详情", tags=["HCP"])
def get_hcp(
    hcp_id: int,
    service: HcpService = Depends(),
    current_user: dict = Depends(require_scope("visit")),
...
@router.patch("/hcp/{hcp_id}", summary="更新HCP", description="更新指定HCP信息", tags=["HCP"])
def update_hcp(
    hcp_id: int,
    body: HcpUpdate,
    service: HcpService = Depends(),
...
@router.delete("/hcp/{hcp_id}", summary="删除HCP", description="删除指定HCP", tags=["HCP"])
def delete_hcp(
    hcp_id: int,
    service: HcpService = Depends(),
    current_user: dict = Depends(require_scope("visit")),
...
@router.post("/products", summary="创建产品", description="创建产品信息", tags=["产品"])
def create_product(
    body: ProductCreate,
    service: HcpService = Depends(),
    current_user: dict = Depends(require_scope("visit")),
...
@router.get("/products", summary="产品列表", description="获取产品列表", tags=["产品"])
def list_products(
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=100),
...
@router.get("/products/{product_id}", summary="产品详情", description="获取指定产品详情", tags=["产品"])
def get_product(
    product_id: int,
    service: HcpService = Depends(),
    current_user: dict = Depends(require_scope("visit")),
...
@router.patch("/products/{product_id}", summary="更新产品", description="更新指定产品信息", tags=["产品"])
def update_product(
    product_id: int,
    body: ProductUpdate,
    service: HcpService = Depends(),
...
@router.delete("/products/{product_id}", summary="删除产品", description="删除指定产品", tags=["产品"])
def delete_product(
    product_id: int,
    service: HcpService = Depends(),
    current_user: dict = Depends(require_scope("visit")),
...
@router.post("/hcp/{hcp_id}/products", summary="创建关联", description="创建HCP与产品关联", tags=["产品"])
def create_relation(
    hcp_id: int,
    body: RelationCreate,
    service: HcpService = Depends(),
...
@router.get("/hcp/{hcp_id}/products", summary="关联列表", description="获取HCP产品关联列表", tags=["产品"])
def list_relations(
    hcp_id: int,
    service: HcpService = Depends(),
    current_user: dict = Depends(require_scope("visit")),
...
@router.delete("/hcp/relations/{relation_id}", summary="删除关联", description="删除指定HCP产品关联", tags=["产品"])
def delete_relation(
    relation_id: int,
    service: HcpService = Depends(),
    current_user: dict = Depends(require_scope("visit")),
